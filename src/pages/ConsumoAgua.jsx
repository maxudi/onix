...
export default function ConsumoAgua({ embedMode }) {
  ...
  const [selectedYear, setSelectedYear] = useState('movel'); // 'movel' ou o número do ano
  const [loading, setLoading] = useState(true);
  const [transactions, setTransactions] = useState([]);

  const years = useMemo(() => Array.from({ length: currentYear - 2022 + 1 }, (_, i) => 2022 + i), [currentYear]);

  // Define os períodos baseados na escolha do usuário
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50/50 min-h-screen">
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-black text-blue-800 uppercase italic">Consumo Água</h1>
      ...
      <div className="bg-white rounded-3xl border border-blue-100 shadow-sm p-6 h-80">
